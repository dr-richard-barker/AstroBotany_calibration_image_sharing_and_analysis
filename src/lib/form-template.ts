...
export const TEMPLATE_FIELDS: TemplateField[] = [
  ...
  { question: 'Calibration marker in frame?', type: 'dropdown', options: ['Yes — AstroBotany ArUco card', 'Yes — ruler / scale bar', 'Yes — colour card', 'No'], required: true, help: 'The field the reviewed projects were missing.' },
...
const PROJECT_REF = 'a57 b0da 71c 4e5 8f60 astrobotany'.replace(/[^a-f0-9]/g, '').padEnd(32, '0').slice(0, 32);
const hex = (n: number) => (n + 0x1000000000000).toString(16).slice(-13);

// Build the Epicollect5 project structure object.
export function buildEc5Template(projectName = 'AstroBotany Calibration Image Metadata') {
  ...
  return {
    data: {
      ...
      project: {
        ...
        description: 'Recommended metadata form generated from the AstroBotany metadata review — includes provenance, controlled organism/light/climate fields with SI units, repeatable measurements, and a calibration-marker field.',
        small_description: 'Recommended AstroBotany calibration-image metadata form.',
