// Real, openly-licensed starter images recycled from Richard Barker's own
// repositories (no stock photos, no fabricated records). Marker analysis is
// left null — run "Detect marker" in the inspector (browser-side CV) to
// populate it. Shared by `npm run seed` and the server's auto-seed-on-empty.
...
// Seed any images that aren't present yet (idempotent). Returns the number added.
export function runSeed(log: (m: string) => void = () => {}): number {
  const seedDir = path.join(ROOT, 'public', 'seed');
  const existing = new Set(
    (db.prepare("SELECT title FROM images WHERE source = 'seed'").all() as Row[]).map(r => r.title),
  );
  let added = 0;
  for (const s of SEEDS) {
    if (existing.has(s.title)) { log(`· skip (already seeded): ${s.title}`); continue; }
    const full = path.join(seedDir, s.file);
    if (!fs.existsSync(full)) { log(`! missing seed file: ${s.file}`); continue; }
    const buffer = fs.readFileSync(full);
    const dims = imageSize(buffer);
    storeImage({
      buffer, mime: s.file.endsWith('.png') ? 'image/png' : 'image/jpeg',
