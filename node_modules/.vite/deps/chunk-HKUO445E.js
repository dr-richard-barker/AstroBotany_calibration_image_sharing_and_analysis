...
// node_modules/js-aruco2/src/cv.js
var require_cv = __commonJS({
  "node_modules/js-aruco2/src/cv.js"(exports) {
    var CV = CV || {};
    exports.CV = CV;
    ...
    CV.grayscale = function(imageSrc, imageDst) {
      ...
      for (; i < len; i += 4) {
        dst[j++] = src[i] * 0.299 + src[i + 1] * 0.587 + src[i + 2] * 0.114 + 0.5 & 255;
      }
    ...
    CV.otsu = function(imageSrc) {
      var src = imageSrc.data, len = src.length, hist = [], threshold = 0, sum = 0, sumB = 0, wB = 0, wF = 0, max = 0, mu, between, i;
      for (i = 0; i < 256; ++i) {
        hist[i] = 0;
      }
      for (i = 0; i < len; ++i) {
    ...
    CV.gaussianBlurFilter = function(imageSrc, imageDst, kernel, horizontal) {
      var src = imageSrc.data, dst = imageDst.data, height = imageSrc.height, width = imageSrc.width, pos = 0, limit = kernel.length >> 1, cur, value, i, j, k;
      for (i = 0; i < height; ++i) {
        for (j = 0; j < width; ++j) {
          value = 0;
          ...
          dst[pos++] = horizontal ? value : value + 0.5 & 255;
        }
      }
      return imageDst;
    };
    ...
    CV.approxPolyDP = function(contour, epsilon) {
      var slice = { start_index: 0, end_index: 0 }, right_slice = { start_index: 0, end_index: 0 }, poly = [], stack = [], len = contour.length, pt, start_pt, end_pt, dist, max_dist, le_eps, dx, dy, i, j, k;
      epsilon *= epsilon;
      k = 0;
      for (i = 0; i < 3; ++i) {
        max_dist = 0;
    ...
    CV.warp = function(imageSrc, imageDst, contour, warpSize) {
      var src = imageSrc.data, dst = imageDst.data, width = imageSrc.width, height = imageSrc.height, pos = 0, sx1, sx2, dx1, dx2, sy1, sy2, dy1, dy2, p1, p2, p3, p4, m, r, s, t, u, v, w, x, y, i, j;
      ...
      for (i = 0; i < warpSize; ++i) {
        ...
        for (j = 0; j < warpSize; ++j) {
          ...
          p1 = p2 = sy1 * width;
          p3 = p4 = sy2 * width;
          dst[pos++] = dy2 * (dx2 * src[p1 + sx1] + dx1 * src[p2 + sx2]) + dy1 * (dx2 * src[p3 + sx1] + dx1 * src[p4 + sx2]) & 255;
        }
      }
    ...
    CV.perimeter = function(poly) {
      var len = poly.length, i = 0, j = len - 1, p = 0, dx, dy;
      for (; i < len; j = i++) {
    ...
    CV.countNonZero = function(imageSrc, square) {
      var src = imageSrc.data, height = square.height, width = square.width, pos = square.x + square.y * imageSrc.width, span = imageSrc.width - width, nz = 0, i, j;
      for (i = 0; i < height; ++i) {
        for (j = 0; j < width; ++j) {
          if (0 !== src[pos++]) {
            ++nz;
...
//# sourceMappingURL=chunk-HKUO445E.js.map
